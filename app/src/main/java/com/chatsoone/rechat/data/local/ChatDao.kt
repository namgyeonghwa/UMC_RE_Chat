...
interface ChatDao {
    ...
    @Query(
        "SELECT CL.chatIdx, CL.chatName AS nickName, CL.profileImg AS profileImg, CL.latestTime AS postTime, CM.message, CM.groupName, CM.isNew\n" +
                "FROM\n" +
    ...
    //갠톡 채팅 가져오기, 검증된
    @Query(
        "SELECT C.chatIdx, OU.nickname as nickName, C.groupName, OU.image as profileImg, C.message, C.postTime, C.isNew\n" +
