...
import com.chatsoone.rechat.data.entity.FolderContent
...
interface FolderContentDao {
    ...
    //갠톡 채팅들 폴더에 추가, 검증
    @Query(
        "INSERT INTO FolderContentTable (folderIdx, chatIdx)\n" +
    ...
    //폴더의 모든 챗 가져오기, 검증
    @Query(
        "SELECT FI.folderName, OU.nickname, OU.image as profileImgUrl, C.message, C.postTime as postTime, C.chatIdx as chatIdx\n" +
    ...
    fun getFolderChat(
        userIdx: Long,
        folderIdx: Int
    ): LiveData<List<com.chatsoone.rechat.data.remote.FolderContent>>
}